...
export const CatalogPreview: React.FC<CatalogPreviewProps> = ({ storeInfo, products, previewRef }) => {
  ...
  return (
    <div className="flex justify-center w-full min-h-screen p-4 md:p-8 bg-slate-200/30">
      <div
        ...
        style={{ minHeight: '1120px', width: '800px', ['--brand-color' as any]: storeInfo.color || '#f97316', }}
      >
        <style>{`
...
`}</style>
        ...
        >
          ...
          <div className={`flex flex-col md:flex-row justify-between items-center gap-6 relative z-10 ${isClassic ? 'text-center md:text-left' : ''}`}>
            <div className={`flex items-center gap-6 ${isClassic ? 'flex-col md:flex-row' : ''}`}>
              {storeInfo.logo && (
                <div className={`${isModern ? 'w-24 h-24 rounded-3xl rotate-3' : isMinimalist ? 'w-16 h-16 rounded-none border border-slate-200' : 'w-20 h-20 rounded-2xl'
              ...
              <div>
                <h1 className={`font-extrabold uppercase tracking-tight ${isMinimalist ? 'text-2xl' : 'text-4xl'
            ...
            {storeInfo.whatsapp && (
              <div className={`flex items-center gap-2 px-6 py-3 rounded-full font-bold ${isMinimalist ? 'bg-slate-100 text-slate-900 border border-slate-200' : 'bg-black/20 text-white border border-white/20'
                } backdrop-blur-md`}>
          ...
          <div className="products-grid grid grid-cols-2  gap-y-12">
            ...
            {products.map((product) => (
              <div key={product.id}
                ...
                <div
                  className={`aspect-[4/3] w-full overflow-hidden shadow-sm relative flex items-center justify-center ${isModern ? 'rounded-[2rem]' : isMinimalist ? 'rounded-none border border-slate-100' : 'rounded-2xl border border-slate-100'
                    }`}
                >
                ...
                <div
                  ...
                  className={`sm:hidden -mt-1 font-bold ${isClassic ? 'text-center' : 'text-left'
                    } ${isMinimalist
                ...
                <div className={isClassic ? 'px-2' : ''}>
                  ...
                  {product.description && (
                    <div
                      ...
                      ${isMinimalist ? 'text-[10px]' : 'text-xs'}
                      catalog-html
